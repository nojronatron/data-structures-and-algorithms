...
const screenForNames = (arr) => {
  // let result = [];
  // let re = /^(Mr\.\s|Mrs\.\s|Ms\.\s|Dr\.\s)/gm;
  // arr.map((word) => {
  //   let innerResult = word.match(re);
  //   if (innerResult) {
...
const toTitleCase = (arr) => {
  ...
  let result = arr.map((words) => {
    return words.replace(firsts, (rawMatch, thisMatch) => {
      return thisMatch.toUpperCase();
    });
  });
  return result;
};
...
let biggerThanLuke = (arr) => {
  // let lukeMass = starWarsData.filter((luke) => luke.name === 'Luke Skywalker')[0].mass;
  // return arr.filter(character => (character.mass > lukeMass));
