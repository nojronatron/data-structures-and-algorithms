...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter((statItem) => statItem.baseStat > minBaseStat);
};
...
const getStatName = (arr, minBaseStat) => {
  // let filtered = getBaseStatGreaterThan(arr, minBaseStat);
  // return filtered.filter(item => item['name'] !== undefined);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter((family) => family.children === undefined);
};
...
const evenOddNumericValues = (arr) => {
  let filteredData = arr.filter( (item) => typeof(item) === 'number');
  let evenOdd = filteredData.map( (num) => (num % 2 === 0) ? 'even' : 'odd');
  return evenOdd;
};
