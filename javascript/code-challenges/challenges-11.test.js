...
function transformToList(obj){
  let result = [];
  Object.keys(obj).forEach(entry => {
    result.push(`<li>${entry}: ${obj[entry]}</li>`);
  });
  return result;
}
...
describe('Testing challenge 1', () => {
  test('It should return a list of key value pairs inside of li tags', () => {
    expect(transformToList({name: 'bob', age: 32})[0]).toStrictEqual(`<li>name: bob</li>`);
    expect(transformToList({name: 'bob', age: 32})[1]).toStrictEqual(`<li>age: 32</li>`);
    expect(transformToList({})).toStrictEqual([]);
  });
});
